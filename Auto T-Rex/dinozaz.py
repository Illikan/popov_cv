...
while True:
    ...
    if player.match(img): 
        ...
        topleft_x_crop = int(player.location[0][0] + player.width)
        ...
        bottomRight_x_dino = int(player.location[1][0]+ player.width)
...
while True:  
    ...
    for i, cnt in enumerate( cnts): 
        x, y, w, h = cv2.boundingRect(cnt)    
        # cv2.rectangle(thresh_crop, (x,y), (x+w, y+h), (255, 255, 255), 2)
        if x < distanceThreshold:
             pag.press('space')     
    player_thresh.match(thresh_dino)
    print(f"{len(cnts_dino)} : {player_thresh.location[0][1]}")
    # if player_thresh.location[0][1] < 107 and player_thresh.location[0][1] != 96   :
    #     time.sleep(0.05)
    #     pag.keyDown('down')
    #     time.sleep(0.05)
    #     pag.keyUp('down')
